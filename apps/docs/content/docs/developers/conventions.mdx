...
## 3. pt-BR translation glossary (Província Labs fork)

Brazilian Portuguese locale lives in `packages/views/locales/pt-BR/`. Landing copy is in `apps/web/features/landing/i18n/pt.ts`.
...
- Brands: Multica, GitHub, Claude, Codex, Cursor, Slack
- Acronyms: API, CLI, URL, OAuth, WebSocket
- Status enums: `backlog`, `todo`, `in_progress`, `in_review`, `done`, `blocked`, `cancelled`
- Roles: `owner`, `admin`, `member`

### Style

- Direct, professional **pt-BR** (not pt-PT)
- Preserve `{{variable}}` placeholders and markdown from the English source
- Use `_one` / `_other` plural forms like English
